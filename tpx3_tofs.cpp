...
int tpx3_tofs(string filename, long nhits=-1) {
   ...
   string ofile = filename.substr(0,filename.size()-5)+"_tof.root";
   ...
   TFile *tofFile = new TFile(ofile.c_str(),"recreate");
   TTree *ttof = new TTree("ttof","Tof data tree");
   //tcl->Branch("n",&npix,"n/I");
   ttof->Branch("x",&xpix,"x/S");
   ttof->Branch("y",&ypix,"y/S");
   ttof->Branch("t",&tpix,"t/L");
   ttof->Branch("e",&epix,"e/S");
   ttof->Branch("tof",&tof,"tof/D");
   ...
   while (nprocessed<nhits) {
     ...
     for (int i=0; i<nsubset; i++) {
       xpix=(Short_t)(x[i]+0.5);
       ypix=(Short_t)(y[i]+0.5);
       epix=(Short_t)(e[i]+0.5);
       tpix=(Long64_t)(t[i]+0.5);

       if (ntrig>0) {
                // calculate ToF only when TDC timestamps are available
                tof = (Double_t) (t[i]*1.5625E-9 - t0find(ntrig, tdc, t[i]*1.5625E-9) ) ;
     ...
     //  }
     //}
     cout << "subsection: " << istep << endl;
         // << " number of clusters found: " << clusid 
         //<< endl;
     istep++;
